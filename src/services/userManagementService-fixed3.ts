...
class UserManagementServiceFixed3 {
  // User Management
  async getUsers(params?: UserSearchParams): Promise<PaginatedUserResponse> {
    try {
      ...
      // Apply search query - Fixed to use proper Supabase syntax with individual .or() calls
      if (params?.query) {
  ...
  // Alternative method using individual .or() calls for search
  async getUsersWithIndividualOr(params?: UserSearchParams): Promise<PaginatedUserResponse> {
    try {
      let query = supabase
        .from("users")
        .select(`
      ...
      // Apply search query using individual .or() calls
      if (params?.query) {
        query = query.or(`first_name.ilike.%${params.query}%`)
                   .or(`last_name.ilike.%${params.query}%`)
      ...
      if (error) {
        console.error("Supabase error in getUsersWithIndividualOr:", error);
        throw error;
      }
      ...
      console.error("Error fetching users (individual or):", error);
      throw new Error("Failed to fetch users");
    }
  }

  // Method without search query for debugging
...
export const userManagementServiceFixed3 = new UserManagementServiceFixed3();
