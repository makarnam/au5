...
const EditEngagementPage: React.FC = () => {
  ...
  const loadEngagement = async (engagementId: string) => {
    try {
      // Note: We need to add getEngagementById method to the service
      const result = await service.getEngagementById?.(engagementId);
      if (result?.data) {
        ...
        setFormData({
          ...
          project_manager_id: engagement.project_manager_id || '',
          contract_manager_id: engagement.contract_manager_id || '',
          contract_number: engagement.contract_number || '',
          contract_value: engagement.contract_value,
          currency: engagement.currency,
          payment_schedule: engagement.payment_schedule || '',
          deliverables: engagement.deliverables || [],
          key_performance_indicators: engagement.key_performance_indicators || [],
          service_level_agreements: engagement.service_level_agreements || '',
          termination_clauses: engagement.termination_clauses || '',
          renewal_terms: engagement.renewal_terms || '',
          risk_mitigation_measures: engagement.risk_mitigation_measures || '',
          compliance_requirements: engagement.compliance_requirements || [],
          security_requirements: engagement.security_requirements || [],
          insurance_requirements: engagement.insurance_requirements || '',
          audit_rights: engagement.audit_rights || '',
          data_processing_agreement: engagement.data_processing_agreement,
          data_processing_activities: engagement.data_processing_activities || [],
          data_retention_period: engagement.data_retention_period,
          data_breach_notification_hours: engagement.data_breach_notification_hours,
          subcontractor_approval_required: engagement.subcontractor_approval_required,
          subcontractors: engagement.subcontractors || [],
        });
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      // Note: We need to add updateEngagement method to the service
      const result = await service.updateEngagement?.(id, formData);
      if (result?.data) {
        navigate('/third-party-risk-management/engagements');
      } else {
        console.error('Error updating engagement:', result?.error);
      }
    } catch (error) {
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Edit Engagement</h1>
          <p className="text-gray-600 mt-2">Update engagement details</p>
        </div>
      </div>
...
export default EditEngagementPage;
