...
import { CMMCManagementFormData } from '../../../types/itSecurity';
import { userManagementService } from '../../../services/userManagementService';
import { User } from '../../../types/userManagement';

const CreateCMMCProgramPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSaving(true);
      const newCMMC = await itSecurityService.cmmcManagement.create(formData);
      navigate(`/it-security/cmmc/${newCMMC.id}`);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to create CMMC program');
    } finally {
      setSaving(false);
    }
  };

  const generateCMMCId = () => {
    const timestamp = new Date().getTime().toString().slice(-6);
    const random = Math.random().toString(36).substring(2, 5).toUpperCase();
    return `CMMC-${timestamp}-${random}`;
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Create CMMC Program</h1>
            <p className="text-gray-600">Set up a new Cybersecurity Maturity Model Certification program</p>
          </div>
        </div>
      </div>
                  ...
                  <div>
                    ...
                    <div className="flex space-x-2">
                      ...
                      <Button
                        type="button"
                        variant="outline"
                        onClick={() => handleInputChange('cmmc_id', generateCMMCId())}
                      >
                        Generate
                      </Button>
                    </div>
...
export default CreateCMMCProgramPage;
