...
import { ArrowLeft, Save, Shield, Target, Activity, BookOpen, Users, Calendar, Loader2 } from 'lucide-react';
...
const EditControlPage: React.FC = () => {
  ...
  const mockControl = {
    ...
    description: 'Implementation of multi-factor authentication for all user accounts to prevent unauthorized access.',
    control_type: 'preventive' as const,
    category: 'access_control' as const,
    status: 'active' as const,
    priority: 'critical' as const,
    implementation_status: 'operational' as const,
    ...
    cost: 15000,
    risk_mitigation: 'Reduces risk of unauthorized access by requiring multiple authentication factors',
    implementation_notes: 'Successfully implemented across all systems. User adoption rate is 95%.',
    testing_procedures: 'Monthly testing of MFA bypass scenarios, quarterly penetration testing',
    documentation: 'MFA Implementation Guide v2.1, User Training Manual',
    tags: ['authentication', 'access-control', 'critical', 'operational']
  };
    ...
    const loadControl = async () => {
      ...
      try {
        ...
        setFormData({
          ...
          description: mockControl.description,
          control_type: mockControl.control_type,
          category: mockControl.category,
          status: mockControl.status,
          priority: mockControl.priority,
          implementation_status: mockControl.implementation_status,
          owner: mockControl.owner,
          responsible_team: mockControl.responsible_team,
          implementation_date: mockControl.implementation_date,
          review_frequency: mockControl.review_frequency,
          last_review_date: mockControl.last_review_date,
          next_review_date: mockControl.next_review_date,
          effectiveness_rating: mockControl.effectiveness_rating,
          cost: mockControl.cost,
          risk_mitigation: mockControl.risk_mitigation,
          implementation_notes: mockControl.implementation_notes,
          testing_procedures: mockControl.testing_procedures,
          documentation: mockControl.documentation,
          tags: mockControl.tags
        });
      } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('Updating control:', formData);
      
      // Simulate API call
      await new Promise(resolve => setTimeout(resolve, 1000));
      
      navigate(`/it-security/controls/${id}`);
    } catch (error) {
    ...
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <Link to={`/it-security/controls/${id}`}>
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Edit Control</h1>
            <p className="text-gray-600">Update control information</p>
          </div>
        </div>
      </div>
