...
export default function ApprovalTimeline({ steps, currentStep }: Props) {
      ...
      {sorted.map((s) => {
        ...
        return (
          <li key={s.id} className="mb-10 ms-6">
            ...
            {s.comments ? <p className="text-sm text-gray-600 whitespace-pre-wrap">Comments: {s.comments}</p> : null}
          </li>
        );
      })}
    </ol>
