...
-- Basic RLS policies (drop existing first to avoid conflicts)
...
DROP POLICY IF EXISTS "Users can view their business unit" ON business_units;
...
DROP POLICY IF EXISTS "Users can view assigned audits" ON audits;
...
-- Add the foreign key constraint for business_units.manager_id after users exist
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints
        WHERE constraint_name = 'fk_business_units_manager'
    ) THEN
...
-- Set up storage bucket for evidence files (simple approach)
DO $$
BEGIN
